...
public final class ApiServer {
  ...
  private static void postCourses(CourseDao courseDao) {
    ...
    app.post("/courses", ctx -> {
      ...
      // We don't really need this check because courseDao.add(course) would throw an exception
      // but, we get warnings when we try to do so because it can't rollback the transaction
      if (course.getName() == null) {
        throw new ApiError("Name should not be null!", 500); // server internal error
      }
      try {
  ...
  private static void postReviewForCourse(ReviewDao reviewDao) {
    ...
    app.post("/courses/:id/reviews", ctx -> {
      ...
      int endpointCourseId = Integer.parseInt(ctx.pathParam("id"));
      int reviewBodyCourseId = review.getCourseId();

      if (endpointCourseId != reviewBodyCourseId) {
        throw new ApiError("Endpoint courseId doesn't match request body's courseId!", 400);
      }
      ...
      try {
        ...
        throw new ApiError(ex.getMessage(), 500); // internal server error
      }
    });
  }
}
